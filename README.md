...
Docker Compose spins up the full stack — backend, frontend, PostgreSQL, and Redis — with no manual setup required.

- **Network** — created automatically; all services reach each other by service name.
- **PostgreSQL volume** — created automatically; data persists between container restarts.

Before starting, create `backend/.env` with your Spotify credentials:

```env
SPOTIFY_CLIENT_ID=...
SPOTIFY_CLIENT_SECRET=...
SPOTIFY_REDIRECT_URI=http://127.0.0.1:8000/auth/callback
DATABASE_URL=postgresql+asyncpg://postgres:password@postgres:5432/moodmap
REDIS_URL=redis://redis:6379
SECRET_KEY=pick_something_random_here
FRONTEND_URL=http://localhost:5173
```

> The hostnames `postgres` and `redis` are Docker Compose service names. For local development **without Docker**, replace them with `localhost`.
...
> PostgreSQL data is stored in a Docker volume and persists between restarts. Running `docker compose down -v` deletes the volume and all stored data.
